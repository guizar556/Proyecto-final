...
$$ LANGUAGE plpgsql;
-----------------------------------------------------
select verificar_detalles_compra_proveedor(4);
select * from detalles_temporales;
-----------------------------------------------------
...
select realizar_compraa(1,'Minorista','Penidiente'); 
select * from compras order by codigo desc;
------------------------------------------------------


--Paso 5: Insertar los detalles de la compra 
CREATE OR REPLACE FUNCTION procesar_detalles_compra(_codigo_proveedor INTEGER)
RETURNS TEXT AS $$
DECLARE
    registro RECORD;
BEGIN
...
------------------------------------------------------------
select procesar_detalles_compra(3);--Id provedor 
select * from detalles_temporales;--1,5       id producto,cantidad 2-7    -- 3-3
select * from compras_y_productos order by codigo desc;--22--24--28--43  id 2 -- 90 3-210+3 = 213
...
BEGIN
    ...
    mensaje := procesar_detalles_compra(_codigo_proveedor);
...
---------------------------------------------------------------------
SELECT gestionar_compra_completa(3, '12', 'Realizada');
---------------------------------------------------------------------
